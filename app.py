...
def venues():
  ...
  for venue_location in venue_locations:
    venue_state_city={"city":venue_location.city,"state":venue_location.state}
...
def search_venues():
  ...
  # search query
  venues=db.session.query(Venue.id,Venue.name).filter(Venue.name.ilike(search_term))
...
def search_artists():
  ...
  # search query
  artists=db.session.query(Artist.id,Artist.name).filter(Artist.name.ilike(search_term))
